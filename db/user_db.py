...
class UserCommands:
    ...
    def get_105_users(self):
        ...
        users = self.users_collection.find({'_id': {'$gte': 0, '$lte': 104}})
        return users
    ...
    async def get_cluster_peers(self, cluster):
        ...
        user_ids = []
        cursor = self.users_collection.find({'cluster': cluster})
    ...
    async def update_user_cluster(self, user_id, cluster):
        ...
        result = await self.users_collection.update_one({'_id': user_id}, {'$set': {'cluster': cluster}})
        return result
    ...
    async def add_user(self, user: UserModel):
        ...
        # Return the user with the new _id
    ...
    async def update_credentials(self, user_id: int, credentials_data: dict):
        current_user = await self.users_collection.find_one({"_id": user_id})
        if not current_user:
            return None
        
        # Perform the update - $set will only modify the specified fields
        update_result = await self.users_collection.find_one_and_update({"_id": user_id}, {"$set": credentials_data}, return_document=ReturnDocument.AFTER)
    ...
    async def update_preferences(self, user_id: int, preferences: PreferencesModel):
        preferences_dict = preferences.model_dump(exclude={"_id"})
        result = await self.users_collection.update_one({"_id": user_id}, {"$set": {"preferences": preferences_dict}})
        return result
    ...
    async def update_favourites(self, user_id: int, operation_data: dict):
        if operation_data["operation"] == "add":
            await self.users_collection.update_one({"_id": user_id}, {"$addToSet": {"favourites": operation_data["place"]}})
        elif operation_data["operation"] == "remove":
            await self.users_collection.update_one({"_id": user_id}, {"$pull": {"favourites": operation_data["place"]}})
    ...
    async def add_trip(self, user_id: int, trip: TripDetails):
        ...
        result = await self.users_collection.update_one({"_id": user_id}, {"$set": {f"savedTrips.{trip_id}": trip_dict}})
        return result
    ...
    async def update_trip(self, user_id: int, trip: TripDetails):
        ...
        result = await self.users_collection.update_one({"_id": user_id}, update_query)
    ...
    async def delete_trip(self, user_id, trip_id):
        result = await self.users_collection.update_one({"_id": user_id}, {"$unset": {f"savedTrips.{trip_id}": ""}})
        if result.modified_count == 0:
